...
public class searchController implements Initializable {
    ...
    public static boolean isFirtsPage;
    public void initialize(URL arg0, ResourceBundle arg1) {
        ...
        Dairport.setText(Filters.departureAP);
        Aairport.setText(Filters.arrivalAP);
        ...
        routesList = logic.calculateRoutes(Filters.departureAP, Filters.arrivalAP, Filters.departureDate, Filters.flightClass, Filters.passengerNo, Filters.isDirect);
        if (routesList == null) return;

        filters = new Filters();

        airlinesList = filters.listAirlines(routesList);
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            try {
                ...
                if (Filters.flightClass == "Economy") {
                    items.setData(airlines.toString(), departure_air, arrival_air, currRoute.flights.get(0).getDepartureTime().toString(),
                    currRoute.flights.get(currRoute.flights.size()-1).getArrivalTime().toString(), Integer.toString(currRoute.totalTime),
                    Integer.toString(currRoute.totalPriceEconomy), flightIDs.toString(), stops.toString());
                }
                else if (Filters.flightClass == "Business") {
                    items.setData(airlines.toString(), departure_air, arrival_air, currRoute.flights.get(0).getDepartureTime().toString(),
                    currRoute.flights.get(currRoute.flights.size()-1).getArrivalTime().toString(), Integer.toString(currRoute.totalTime),
                    Integer.toString(currRoute.totalPriceBusiness), flightIDs.toString(), stops.toString());
                }
                else if (Filters.flightClass == "First Class") {
                    items.setData(airlines.toString(), departure_air, arrival_air, currRoute.flights.get(0).getDepartureTime().toString(),
                    currRoute.flights.get(currRoute.flights.size()-1).getArrivalTime().toString(), Integer.toString(currRoute.totalTime),
                    Integer.toString(currRoute.totalPriceFirstClass), flightIDs.toString(), stops.toString());
                }
    ...
    public static void label_error(){
        login_err.setVisible(true);
    }
